...
export function DeliveryEdit() {
  ...
  useEffect(() => {
    if (delivery) {
      setFormData({
        ...
        customer_id: delivery.customer_id || 0,
        warehouse_id: delivery.warehouse_id,
        location_id: delivery.location_id || 0,
        notes: "",
      })
      if (delivery.lines) {
        setLines(
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => deliveriesAPI.update(Number(id!), data),
    onSuccess: () => {
  ...
  return (
    <div>
      <PageHeader
        title={`Edit Delivery - ${delivery.reference}`}
        breadcrumbs={[
          { label: "Home", href: "/" },
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button type="submit" disabled={mutation.isPending}>
                {mutation.isPending ? "Updating..." : "Update Delivery"}
              </Button>
            </div>
          </form>
        </CardContent>
      ...
      {/* Add Product Modal - Same as ReceiptEdit */}
      <Modal
