...
import type { CreateTransferRequest, TransferLine } from "@/types"
...
export function TransferForm() {
  ...
  const { data: fromLocationsData } = useQuery({
    queryKey: ["locations", formData.from_location_id],
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    // This is simplified - in real app, you'd filter locations by warehouse
                    setFormData({ ...formData, from_location_id: Number(e.target.value) })
                  }}
                >
                  <option value="0">Select location</option>
                  {fromLocationsData?.locations?.map((loc) => (
              ...
              <div>
                ...
                >
                  <option value="0">Select location</option>
                  {toLocationsData?.locations?.map((loc) => (
                    <option key={loc.id} value={loc.id}>
                      {loc.warehouse_name} - {loc.name}
                    </option>
                  ))}
