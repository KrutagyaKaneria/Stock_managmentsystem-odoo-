...
export default function DeliveryForm({ products, warehouses, locations, onSubmit, onCancel }) {
  const [formData, setFormData] = useState({
    reference: `DEL-${Date.now()}`,
    customerId: null,
    warehouseId: '',
    locationId: '',
    lines: [{ product_id: '', qtyOrdered: '', uom: 'pcs' }]
  })

  const filteredLocations = locations.filter(
    l => l.warehouse_id === formData.warehouseId || l.warehouseId === formData.warehouseId
  )

  function addLine() {
    setFormData({
      ...formData,
      lines: [...formData.lines, { product_id: '', qty_demanded: 1, uom: 'pcs' }]
    })
  }
  ...
  function handleSubmit(e) {
    e.preventDefault()
    if (formData.lines.length === 0 || !formData.warehouseId) {
      alert('Please select warehouse and add at least one product')
      return
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="border-t pt-4">
        <div className="flex items-center justify-between mb-3">
          <label className="text-sm font-medium text-gray-700">Delivery Lines</label>
          <button
            type="button"
            onClick={addLine}
          ...
          {formData.lines.map((line, index) => (
            <div key={index} className="flex gap-2 items-center">
              ...
              <input
                ...
                onChange={(e) => updateLine(index, 'qtyOrdered', Number(e.target.value))}
